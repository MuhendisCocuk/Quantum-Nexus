...
window.addEventListener('scroll', () => {
  ...
  if (header) { // Header elementi varsa işlemi yap
...
function createStars() {
  ...
  if (!galaxyBg) { // galaxy-bg elementi yoksa fonksiyonu çalıştırma
    console.warn("Kaptan Pengu, 'galaxy-bg' elementi bulunamadı. Yıldız efekti oluşturulamıyor.");
    return;
  }
...
// Düzeltme: JSON dosya adları doğru şekilde virgülle ayrıldı
const jsonFiles = ['data1.json', 'data2.json', 'data3.json', 'data4.json', 'data5.json', 'data6.json'];
...
function loadMoreNews() {
  // Haber listesi veya ana sayfa içeriği görünür değilse yükleme yapma
  if (!newsList || homepageContent.style.display === 'none') return;

  // Yükleyici ve haber listesi elementlerinin varlığını kontrol et
  if (!loader || !newsList) {
    console.warn("Kaptan Pengu, haber yükleme için gerekli elementler (loader veya newsList) bulunamadı.");
    return;
  }
...
function loadTrendTopics() {
  if (!trendList) { // trendList elementi yoksa fonksiyonu çalıştırma
    console.warn("Kaptan Pengu, 'trendList' elementi bulunamadı. Trend konuları yüklenemiyor.");
    return;
  }
  ...
  trendTopics.forEach(topic => {
    ...
    tag.addEventListener("click", () => {
      ...
      if (searchInput) { // searchInput elementi varsa
...
function displaySearchResults(query) {
  ...
  if (!homepageContent || !searchResultsContent || !resultsDiv || !searchLoader) {
    console.warn("Kaptan Pengu, arama sonuçları için gerekli elementler (homepageContent, searchResultsContent, resultsDiv, searchLoader) bulunamadı.");
    return;
  }
...
function displayHomepage() {
  if (homepageContent && searchResultsContent) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!searchInput || !searchBtn || !newsList || !loader || !trendList || !resultsDiv || !searchLoader || !suggestionBox || !homepageContent || !searchResultsContent) {
    console.error("Kaptan Pengu, gerekli DOM elementlerinden bazıları bulunamadı. Lütfen HTML yapısını kontrol edin.");
    // Gerekli elementler yoksa daha fazla işlem yapmaktan kaçın
    return;
  }
  ...
  searchInput.addEventListener("keypress", (e) => {
    if (e.key === "Enter") {
      ...
      if (input !== "") {
        ...
        alert("Kaptan Pengu, bir arama terimi girmelisin!"); // Düzeltme: Yazım hatası giderildi
      }
    }
  });
  ...
  searchInput.addEventListener("blur", () => {
    setTimeout(() => {
      if (suggestionBox) { // suggestionBox elementi varsa
  ...
  searchInput.addEventListener("focus", () => {
    if (searchInput && suggestionBox) { // Elementler varsa
      const query = searchInput.value.trim().toLowerCase();
      // Düzeltme: Sadece inputta metin varsa ve öneri kutusunda içerik varsa göster
  ...
  searchInput.addEventListener("input", () => {
    if (!searchInput || !suggestionBox) return; // Elementler yoksa çık
...
window.addEventListener("scroll", () => {
  ...
  // Düzeltme: homepageContent kontrolü eklendi
  if (homepageContent && homepageContent.style.display === 'block' && (window.innerHeight + window.scrollY) >= (document.body.offsetHeight - 150)) {
    loadMoreNews();
  }
});

// Yıldız efektini başlangıçta çağır
// createStars(); // DOMContentLoaded içinde çağrılmalı, elementler hazır olmalı